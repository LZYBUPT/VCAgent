...
def load_psa_scatac_dataset(data_path: str) -> Tuple[List[Dict], List[Dict], List[Dict]]:
    ...
    # Split into train/val/test (90/10/0)
    n_total = len(data)
    n_train = int(0.9 * n_total)
    n_val = int(0.1 * n_total)  # 10% validation split to ensure enough samples for plotting
...
data_path = os.path.join(_ROOT, "data", "Hs 766T", "DE_train.json")
train_data, val_data, test_data = load_psa_scatac_dataset(data_path)
output_path = os.path.join(_ROOT, "output", "iteration100", "Hs 766T_DE_new.json")
...
gepa_result = gepa.optimize(
    ...
    candidate_selection_strategy = "bandit",
    mcts_tree=mcts_tree,
    run_dir=os.path.join(_ROOT, "output", "iteration100", "Hs 766T_DE_new"),
