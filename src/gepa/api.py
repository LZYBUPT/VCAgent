...
def optimize(
    ...
    candidate_selection_strategy: CandidateSelector | Literal["pareto", "current_best", "epsilon_greedy", "bandit"] = "pareto",
    ...
    # Checkpoint configuration
    - enable_checkpoints: Whether to enable checkpoint saving. Defaults to True.
    - checkpoint_every: Save checkpoint every N iterations. Defaults to 5.
    - keep_last_checkpoints: Number of recent checkpoints to keep. Defaults to 3.
    """
    ...
    if isinstance(candidate_selection_strategy, str):
        factories = {
            ...
            "bandit": lambda: BanditCandidateSelector(rng=rng),
        }
        try:
            ...
            raise ValueError(
                ...
                "Supported strategies: 'pareto', 'current_best', 'epsilon_greedy', 'bandit'"
            ) from exc
    ...
    # The module selector determines which specific component to select for reflection and optimization
    ...
    # If an MCTS tree is provided, use MCTS mode; otherwise use traditional mode
    if mcts_tree is not None:
        # MCTS mode: candidate_selector is not used (MCTS replaces candidate selection)
        actual_candidate_selector = None
        actual_mcts_tree = mcts_tree
    else:
        actual_candidate_selector = candidate_selector
        actual_mcts_tree = None

    reflective_proposer = ReflectiveMutationProposer(
        logger=logger,
        trainset=train_loader,
        adapter=active_adapter,
        candidate_selector=actual_candidate_selector,
        module_selector=module_selector_instance,
        batch_sampler=batch_sampler,
        perfect_score=perfect_score,
        skip_perfect_score=skip_perfect_score,
        experiment_tracker=experiment_tracker,
        mcts_tree=actual_mcts_tree,
