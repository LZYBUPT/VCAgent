...
class ContainsAnswerEvaluator:
    ...
    def __call__(self, data: DefaultDataInst, response: str) -> EvaluationResult:
        ...
        # Build feedback as the procedural record described in the paper
        if is_correct:
...
class DefaultAdapter(GEPAAdapter[DefaultDataInst, DefaultTrajectory, DefaultRolloutOutput]):
    ...
    def evaluate(
        ...
        # System prompt is the first value in the candidate dictionary
        ...
        # Returns the evaluation results for the original dataset: per-question scores under
        # the same prompt, along with trajectories and multi-objective scores.
        # The latter two are typically empty in basic usage.
    ...
    # Build reflective data from minibatch evaluation results
